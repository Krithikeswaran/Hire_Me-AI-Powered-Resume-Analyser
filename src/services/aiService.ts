...
import { simpleGeminiAnalyzer } from './simpleGeminiService';
...
const hasGeminiKey = () => {
  return simpleGeminiAnalyzer.isAvailable();
};
...
// Batch analysis function for multiple resumes with comparative ranking
export const analyzeBatchResumes = async (resumeFiles: File[], jobDescription: any): Promise<BatchAnalysisResult> => {
  try {
    console.log(`🚀 Starting batch analysis of ${resumeFiles.length} resumes...`);

    // Convert job description to the format expected by batch service
    const formattedJobDescription = {
      jobTitle: jobDescription.jobTitle || 'Software Developer',
      department: jobDescription.department || 'Engineering',
      experienceLevel: jobDescription.experienceLevel || 'Mid-level',
      minExperience: parseInt(jobDescription.minExperience) || 0,
      maxExperience: parseInt(jobDescription.maxExperience) || 10,
      requiredSkills: typeof jobDescription.requiredSkills === 'string'
        ? jobDescription.requiredSkills.split(',').map((s: string) => s.trim())
        : jobDescription.requiredSkills || [],
      preferredSkills: typeof jobDescription.preferredSkills === 'string'
        ? jobDescription.preferredSkills.split(',').map((s: string) => s.trim())
        : jobDescription.preferredSkills || [],
      education: jobDescription.education || 'Bachelor\'s degree',
      jobDescription: jobDescription.jobDescription || 'Software development role',
      responsibilities: jobDescription.responsibilities || ['Software development', 'Code review', 'Testing']
    };

    // Use batch analysis service for comprehensive analysis
    const result = await batchAnalysisService.processBatch(resumeFiles, formattedJobDescription);
...
export const analyzeResume = async (resumeText: string, jobDescription: any, resumeFile?: File) => {
  try {
    ...
    // Priority 1: Use Gemini if API key is available (highest accuracy)
    if (hasGeminiKey()) {
      console.log('🌟 Using Gemini Pro for enhanced analysis...');
      try {
        const geminiResult = await simpleGeminiAnalyzer.analyzeResume(resumeText, jobDescription as JobDescription);
        console.log('✅ Gemini analysis completed with high accuracy:', geminiResult);
        return geminiResult;
      } catch (geminiError) {
        console.warn('⚠️ Gemini analysis failed, falling back to local analysis:', geminiError);
        // Continue to fallback options below
      }
    }

    // Priority 2: Use LangChain local analysis if available
...
const calculateSkillsMatch = (resumeText: string, requiredSkills: string): number => {
  ...
  console.log('Against resume text (first 500 chars):', resumeLower.substring(0, 500));
  console.log('Full resume text length:', resumeLower.length);
  ...
  skills.forEach(skill => {
    ...
    const skillVariations = [
      ...
      skill.replace(/\s+/g, '-'), // Replace spaces with hyphens
      skill.replace(/\s+/g, '_'), // Replace spaces with underscores
    ];
    ...
    const synonyms = {
      ...
      'large language models': ['llms', 'llm', 'large-language-models'],
      ...
      'web development': ['web-development', 'webdev', 'web dev'],
      'frontend development': ['frontend', 'front-end', 'front end'],
      'ui/ux': ['ui', 'ux', 'user interface', 'user experience'],
      'qgis': ['quantum gis', 'q-gis'],
      'arcgis': ['arc gis', 'arc-gis', 'esri'],
      'google colab': ['colab', 'colaboratory', 'google colaboratory']
    };
    ...
    for (const variation of skillVariations) {
      ...
      if (resumeLower.includes(variation)) {
        ...
        // Try word boundary matching for better accuracy
        const wordBoundaryRegex = new RegExp(`\\b${variation.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
        if (wordBoundaryRegex.test(resumeLower)) {
          skillMatched = true;
        }
      }

      if (skillMatched && !matchedSkills.includes(skill)) {
        matchCount++;
        ...
        const occurrences = (resumeLower.match(new RegExp(variation.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')) || []).length;
        totalScore += Math.min(occurrences * 2, 10);

        console.log(`Matched skill: ${skill} (variation: ${variation})`);
        break;
      }
    }
  });
  ...
  // Debug: Check for specific skills that should be in your resume
  const debugSkills = ['html', 'css', 'javascript', 'python', 'mysql', 'machine learning', 'deep learning', 'power bi'];
  console.log('Debug - Checking for specific skills in resume:');
  debugSkills.forEach(debugSkill => {
    const found = resumeLower.includes(debugSkill) || resumeLower.includes(debugSkill.replace(/\s+/g, ''));
    console.log(`  ${debugSkill}: ${found ? 'FOUND' : 'NOT FOUND'}`);
  });
...
const calculateTechnicalFit = (resumeText: string, jobTitle: string, jobDescription: string): number => {
  ...
  const technicalKeywords = {
    frontend: ['html', 'css', 'javascript', 'react', 'vue', 'angular', 'typescript', 'sass', 'webpack', 'responsive', 'ui', 'ux'],
    backend: ['api', 'database', 'server', 'node.js', 'python', 'java', 'sql', 'mongodb', 'postgresql', 'microservices', 'mysql'],
    fullstack: ['frontend', 'backend', 'full-stack', 'javascript', 'react', 'node.js', 'database', 'api', 'html', 'css', 'python', 'mysql'],
    ...
    data: ['python', 'sql', 'pandas', 'numpy', 'machine-learning', 'tensorflow', 'pytorch', 'data-analysis', 'data-science', 'deep-learning', 'llm', 'nlp', 'hugging-face', 'power-bi', 'data-visualization', 'mysql', 'colab', 'transformers', 'artificial-intelligence'],
    qa: ['testing', 'automation', 'selenium', 'cypress', 'junit', 'test-cases', 'quality-assurance']
  };

  // Detect job role with improved data science detection
  let detectedRole = 'fullstack'; // default
  if (jobText.includes('data') || jobText.includes('analyst') || jobText.includes('scientist') ||
      jobText.includes('machine learning') || jobText.includes('ml') || jobText.includes('ai') ||
      jobText.includes('deep learning') || jobText.includes('nlp') || jobText.includes('analytics') ||
      jobText.includes('visualization') || jobText.includes('power bi') || jobText.includes('pytorch')) {
    detectedRole = 'data';
  } else if (jobText.includes('frontend') || jobText.includes('front-end') || jobText.includes('ui')) {
