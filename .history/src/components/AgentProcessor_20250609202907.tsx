...
import { Brain, Users, FileSearch, UserCheck, TrendingUp, CheckCircle, AlertCircle, Loader2, Settings } from "lucide-react";
import { toast } from "@/hooks/use-toast";
import { initializeAI, extractResumeText, analyzeResume, setAIModel, getCurrentAIModel, type AIModel } from "@/services/aiService";
...
const AgentProcessor = ({ resumes, jobDescription, onComplete, isProcessing, setIsProcessing }) => {
  ...
  const handleModelSelect = (model: AIModel) => {
    ...
    setAIModel(model);
    addLog(`🔄 AI model switched to: ${model}`, 'info');
  };
  ...
  const agents = [
    {
      ...
      icon: Brain,
      description: `Loading ${selectedModel === 'local' ? 'local' : selectedModel === 'gemini' ? 'Gemini Pro' : 'OpenAI GPT-4'} models`,
      color: "text-blue-600"
    },
    {
  ...
  const processResumesWithAI = async () => {
    ...
    try {
      // Step 1: Initialize AI with selected model
      setCurrentAgent(0);
      addLog(`🚀 Starting AI initialization with ${selectedModel} model...`, 'info');

      const startTime = Date.now();
      const aiReady = await initializeAI(selectedModel);
      const initTime = Date.now() - startTime;
      
      if (aiReady) {
        addLog(`✅ ${selectedModel === 'local' ? 'Local AI' : selectedModel === 'gemini' ? 'Gemini Pro' : 'OpenAI GPT-4'} models loaded successfully (${(initTime/1000).toFixed(1)}s)`, 'success');
        setAiInitialized(true);
      } else {
        addLog(`⚠️ ${selectedModel} initialization failed, using enhanced fallback analysis`, 'warning');
      }
      setProgress(20);
      ...
      // Step 3: AI Analysis with selected model
      setCurrentAgent(2);
      addLog(`🤖 Running ${selectedModel === 'local' ? 'local AI' : selectedModel === 'gemini' ? 'Gemini Pro' : 'OpenAI GPT-4'} candidate analysis...`, 'info');
      ...
      const results = {
        ...
        summary: {
          ...
          processingTime: aiInitialized ? `${selectedModel === 'local' ? 'Local AI' : selectedModel === 'gemini' ? 'Gemini Pro' : 'OpenAI GPT-4'} Analysis` : "Smart Fallback Analysis",
  ...
  useEffect(() => {
    // Initialize with current model selection
    setSelectedModel(getCurrentAIModel());
  }, []);

  // Don't auto-start processing - wait for user to select model and start

  return (
    <div className="space-y-6">
      {/* Model Selection */}
      {showModelSelector && !processingStarted && (
        <div className="space-y-6">
          <ModelSelector
        ...
        <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
          <CardHeader className="text-center pb-4">
            ...
            <p className="text-muted-foreground">
              Analyzing {resumes.length} resumes with {selectedModel === 'local' ? 'local AI' : selectedModel === 'gemini' ? 'Gemini Pro' : 'OpenAI GPT-4'} models
              {currentAgent === 2 && ` (${currentResumeIndex}/${resumes.length})`}
            </p>
            <div className="flex items-center justify-center gap-2 text-sm text-blue-600 bg-blue-50 p-2 rounded">
              <Brain className="w-4 h-4" />
              Using {selectedModel === 'local' ? 'local models for privacy' : selectedModel === 'gemini' ? 'Gemini Pro for enhanced accuracy' : 'OpenAI GPT-4 for maximum precision'}
            </div>
            {!isProcessing && (
              <Button
