...
import { langchainAnalyzer } from './langchainService';
...
let selectedModel: AIModel = 'local'; // Default to local models
...
// Function to get current AI model
export const getCurrentAIModel = (): AIModel => {
  return selectedModel;
};

export const initializeAI = async (model?: AIModel) => {
  if (model) {
    selectedModel = model;
  }
  try {
...
export const analyzeResume = async (resumeText: string, jobDescription: any, resumeFile?: File) => {
  try {
    console.log(`🔍 Analyzing resume for ${jobDescription.jobTitle} using ${selectedModel} model...`);
    ...
    switch (selectedModel) {
      case 'gemini':
        console.log('🌟 Using Google Gemini Pro analysis...');
        try {
          const geminiResult = await geminiAnalyzer.analyzeResumeWithGemini(resumeText, jobDescription as JobDescription);
      ...
      case 'openai':
        console.log('🧠 Using OpenAI GPT-4 analysis...');
        try {
          const openaiResult = await langchainAnalyzer.analyzeResumeWithLangChain(resumeFile || new File([resumeText], 'resume.txt'), jobDescription as JobDescription);
          console.log('✅ OpenAI analysis completed:', openaiResult);
          return openaiResult;
        } catch (openaiError) {
      ...
      default:
        ...
        if (useLangChain && resumeFile) {
          ...
          try {
            ...
            console.log('✅ LangChain local analysis completed:', langchainResult);
            return langchainResult;
          } catch (langchainError) {
            console.warn('LangChain local analysis failed, falling back to basic analysis:', langchainError);
