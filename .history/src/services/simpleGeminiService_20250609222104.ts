...
class SimpleGeminiAnalyzer {
  ...
  private calculateSkillsMatch(resumeText: string, requiredSkills: string[]): number {
    ...
    // Enhanced skill variations mapping with more comprehensive coverage
    const skillMap = {
    ...
    requiredSkills.forEach(skill => {
      ...
      if (found) {
        matchCount++;
        console.log(`✅ Found skill match: ${skill}`);
      } else {
    ...
    console.log(`📊 Skills match: ${matchCount}/${requiredSkills.length} (${Math.round(matchPercentage)}%)`);
