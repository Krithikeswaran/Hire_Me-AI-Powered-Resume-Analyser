...
export const initializeAI = async () => {
  try {
    ...
    if (hasGeminiKey()) {
      console.log('🌟 Gemini API key detected - will use Gemini Pro for enhanced accuracy!');
      console.log('✅ Gemini Pro ready for high-accuracy analysis');
      return true;
    } else {
...
export const analyzeResume = async (resumeText: string, jobDescription: any, resumeFile?: File) => {
  try {
    ...
    // Priority 1: Use Gemini if API key is available (best accuracy)
    if (hasGeminiKey()) {
      console.log('🌟 Using Google Gemini Pro for enhanced analysis...');
      try {
        ...
        if (geminiResult && typeof geminiResult.overallScore === 'number') {
          ...
          console.warn('⚠️ Gemini returned invalid result format, falling back');
