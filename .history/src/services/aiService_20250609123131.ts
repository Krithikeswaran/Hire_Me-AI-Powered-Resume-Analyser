...
export const extractResumeText = async (file: File): Promise<string> => {
  try {
    ...
    // Use Local AI PDF extraction if available
    if (useLocalAI && file.type === 'application/pdf') {
      console.log('📄 Using Local AI PDF extraction...');
      return await localAIAnalyzer.extractResumeText(file);
    }
...
export const analyzeResume = async (resumeText: string, jobDescription: any, resumeFile?: File) => {
  try {
    ...
    // Use Advanced Local AI analysis if available
    if (useLocalAI && resumeFile) {
      console.log('🤖 Using Advanced Local AI analysis...');
      try {
        const localAIResult = await localAIAnalyzer.analyzeResume(resumeFile, jobDescription);
        console.log('✅ Local AI analysis completed:', localAIResult);
        return localAIResult;
      } catch (localAIError) {
        console.warn('Local AI analysis failed, falling back to basic analysis:', localAIError);
        // Continue with fallback analysis below
      }
    }
