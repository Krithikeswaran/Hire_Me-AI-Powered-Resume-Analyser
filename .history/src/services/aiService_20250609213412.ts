...
export const analyzeResume = async (resumeText: string, jobDescription: any, resumeFile?: File) => {
  try {
    ...
    if (hasGeminiKey() && resumeFile) {
      console.log('🌟 Using Gemini Pro for comprehensive analysis...');
      try {
        const geminiResult = await geminiAnalyzer.analyzeResumeWithGemini(resumeFile, jobDescription as JobDescription);
        ...
        if (geminiResult && typeof geminiResult.overallScore === 'number') {
          ...
          console.warn('⚠️ Gemini returned invalid result, falling back');
